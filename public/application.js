...
$(function () {
  ...
  $('.signInClose').on('click', events.signInForm);
...
var gameLogic = [null, null, null, null, null, null, null, null, null];

var ids = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight'];

var over = false;

var indexExport = null;
...
// Win combos
var wins = [[0, 1, 2], [0, 3, 6], [0, 4, 8], [2, 5, 8], [2, 4, 6], [3, 4, 5], [6, 7, 8], [1, 4, 7]];

// Checks gameLogic array for win combo matches and logs a win message
var checker = function checker(wins) {
  var holder = [gameLogic[wins[0]], gameLogic[wins[1]], gameLogic[wins[2]]];
  if (holder.every(function (value) {
    return value === 'X';
  })) {
    $('#winDisplay').text('X wins!');
    over = true;
  } else if (holder.every(function (value) {
    return value === 'O';
  })) {
...
var clickDisplay = function clickDisplay() {
  if (over === true) {
...
var signUpReset = function signUpReset() {
  $('.failedSignUp').text('');
...
var onCreateGame = function onCreateGame(event) {
  event.preventDefault();
  authApi.createGame().then(ui.createGameSuccess).catch(ui.createGameFail);
};
...
var onUpdateGame = function onUpdateGame() {
  ...
  authApi.updateGame(gameData).then(ui.updateGameSuccess).catch(ui.createGameFail);
};

var onGetGames = function onGetGames() {
  authApi.getGames().then(ui.getGamesSuccess).catch(ui.getGamesFail);
};
...
var updateGameSuccess = function updateGameSuccess(updateGameResponse) {
...
module.exports = __webpack_require__.p + "5b833fe341087995ad71c5225424fc46.jpg";
